...
class Paciente:
    ...
    def verificadorTelefono(self, valor, mensaje): #Expresiones regulares
        if not valor or not re.match(r"\([0-9]{3}\)[0-9]{7}", valor): #Para verificar que el formato de número de teléfono sea correcto
            raise ValueError(mensaje)
        return valor

    def verificadorAltuPeso(self, valor, mensaje):
        if not valor or not re.match("^[0-9]{1,3}$", valor): #Para verificar que el formato de peso sea correcto
            raise ValueError(mensaje)
        return valor
